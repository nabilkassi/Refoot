    ...
    <Link href={`/annonce/${listing.id}`} style={{ textDecoration: 'none' }}>
      ...
      >
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: fav ? 'rgba(239,68,68,0.5)' : 'rgba(0,0,0,0.5)',
              border: '1px solid var(--border)',
        ...
        <div style={{ padding: '10px 10px 12px' }}>
          <div style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '1px', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '2px' }}>
            {listing.brand}
          </div>
          <div style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontWeight: 600, fontSize: '13px', lineHeight: 1.3, marginBottom: '6px', color: 'var(--text)' }}>
            {listing.model}
          </div>
          <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '8px' }}>
            ...
            {listing.playStyle !== 'all' && (
              <span style={{ background: 'rgba(255,255,255,0.06)', color: 'var(--text-muted)', fontSize: '10px', padding: '2px 7px', borderRadius: '4px' }}>
                {listing.playStyle === 'speed' ? '⚡ Vitesse' : listing.playStyle === 'control' ? '🎯 Contrôle' : listing.playStyle === 'power' ? '💪 Puissance' : '🔄 Polyval.'}
              </span>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <span style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontWeight: 700, fontSize: '16px', color: 'var(--text)' }}>
                {listing.price} €
              </span>
              {isUsed && listing.originalPrice && (
                <span style={{ fontSize: '11px', color: 'var(--text-muted)', textDecoration: 'line-through', marginLeft: '4px' }}>
                  {listing.originalPrice} €
                </span>
              )}
            </div>
            <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>T.{listing.size}</span>
          </div>
        </div>
      </article>
    </Link>
